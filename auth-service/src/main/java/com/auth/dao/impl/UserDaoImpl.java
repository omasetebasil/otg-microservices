...
public class UserDaoImpl implements UserDao {
	...
	@SuppressWarnings("deprecation")
	@Override
	public User findUserByMobilenumber(String username) {
		User user = null;
		try (Session openSession = entityManger.unwrap(SessionFactory.class).openSession()) {
			user = (User) openSession.createCriteria(User.class).add(Restrictions.eq("mobilenumber", username)).uniqueResult();
			openSession.beginTransaction().commit();
		} catch (HibernateException e) {
			e.printStackTrace();
		}
